...
namespace ssAppModels.ApiModels
{
   ...
   {
      // 共通リクエスト生成メソッド
      private static RakutenSearchOrderRequest CreateRequest(List<int> orderProgressList, List<int> subStatusIdList, int dateType, int startDaysOffset)
      {
         return new RakutenSearchOrderRequest
         {
            ...
            DateType = dateType,
            StartDatetime = DateTime.Now.AddDays(startDaysOffset).ToString("yyyy-MM-ddTHH:mm:ss+0900"),
            EndDatetime = DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss+0900")
         };
      }
      // 注文確認中リクエスト
      public static RakutenSearchOrderRequest ConfirmingOrderRequest()
      {
         return CreateRequest(new List<int> { 200 }, new List<int> { -1 }, 1, -30);
      }
      // 新規注文リクエスト
      public static RakutenSearchOrderRequest NewOrderRequest()
      {
         return CreateRequest(new List<int> { 300 }, new List<int> { -1 }, 1, -30);
      }
      // 発送処理中リクエスト
      public static RakutenSearchOrderRequest ShippingProcessingRequest()
      {
         return CreateRequest(new List<int> { 300 }, new List<int> { 273015 }, 1, -30);
      }
      // 出荷完了リクエスト
      public static RakutenSearchOrderRequest ShippedOrderRequest()
      {
         return CreateRequest(new List<int> { 500 }, new List<int> { 268493 }, 5, -30);
      }
      // プレゼント対象注文リクエスト
      public static RakutenSearchOrderRequest PresentTargetOrderRequest()
      {
         return CreateRequest(new List<int> { 500 }, new List<int> { 268493 }, 1, -60);
      }
   }
